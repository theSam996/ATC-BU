...
// Fallback heart path if icons not in expected format
const FALLBACK_HEART = [
  ['path', { key: 'h1', d: 'M12 21.35l-1.45-1.32C5.4 15.36 2 12.28 2 8.5 2 5.42 4.42 3 7.5 3c1.74 0 3.41.81 4.5 2.09C13.09 3.81 14.76 3 16.5 3 19.58 3 22 5.42 22 8.5c0 3.78-3.4 6.86-8.55 11.54L12 21.35z' }]
];

const ICONS = {
  heart: FavouriteIcon || FALLBACK_HEART,
  star: StarIcon,
  thumb: ThumbsUpIcon
};
...
export default function PulseHeart({
  ...
  const paths = typeof icon === 'string' ? ICONS[icon] || ICONS.heart || FALLBACK_HEART : null;
        ...
        <span ref={heartRef} className="pulse-heart__heart" aria-hidden="true">
          {Array.isArray(paths) ? (
            <svg viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
              <g ref={glyphRef}>
                {paths.map((item, idx) => {
                  const attrs = Array.isArray(item) ? item[1] : item;
                  return (
                    <path
                      key={String(attrs.key || idx)}
                      d={String(attrs.d || '')}
