...
export default function TearTicket({
  ...
  const is3D = tilt && perspective > 0;
  const plane = is3D
    ? useMotionTemplate`perspective(${perspective}px) rotate(${rotate}deg) rotateX(${tiltX}deg) rotateY(${tiltY}deg)`
    : rotate !== 0
      ? useMotionTemplate`rotate(${rotate}deg)`
      : undefined;
  const depth = tilt && tiltMax > 0 ? parallax / tiltMax : 0;
